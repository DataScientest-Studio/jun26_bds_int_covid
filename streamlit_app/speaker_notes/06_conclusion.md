...
So the next steps would be: test on images from hospitals that are not in this dataset. Our results make a  prediction—COVID recall should drop the most. Beyond that, we’d train a model to predict the source directly, penalise source information during training, and, above all, build datasets where each hospital contributes more than one class. 
