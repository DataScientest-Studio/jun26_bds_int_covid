...
For Dokploy, create a Compose application and point it at `streamlit_app/docker-compose.yml` in this repository. The build context is the repository root, so the image includes `src/`, the four demo models, report metrics, figures, the PDF, and bundled example X-rays with lung masks in `streamlit_app/assets/examples/` (masks live in `examples/masks/`). Raw dataset files and training checkpoints are excluded via `.dockerignore`.
