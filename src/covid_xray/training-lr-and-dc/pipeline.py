...
from .evaluation import EvaluationResult, evaluate_model, format_evaluation, plot_confusion_matrix, save_metrics
...
def run_baseline(
    ...
    for name, model in build_baseline_models(config).items():
        ...
        evaluations[name] = {
            split_name: evaluate_model(model, X, y, name, split_name)
    ...
    if save:
        for name, split_evals in evaluations.items():
            save_metrics(split_evals, reports_dir / f"{name}_metrics.json")
            for split_name, result in split_evals.items():
                plot_confusion_matrix(
                    result, reports_dir / f"{name}_{split_name}_confusion_matrix.png"
                )

    return BaselineResult(splits=splits, evaluations=evaluations, model_paths=model_paths)
